...
class Xml_neworder(Command):
	...
	#action format: "B" or "S"
		...
		<quantity>{3}</quantity>\
		<buysell>{4}</buysell>\
		</command>'.format(board, seccode, price_tag, lots, action, client) #validafter = 0: immediate validity
...
class Connector():
	def __init__(self, new=False):
		...
		ctypes.windll.LoadLibrary("C://Programming/Python/projects/connector/%s" % dll)
		...
		self.securities = {}
		self.rouble_account = {'client_id':'id',
		...
		self.demo_account = {'client_id':'id',
			'portfolio_client':'id2', #for getting the portfolio
			'trade_client':'id3', #for trading
			'password':'password',
			'HOST':'78.41.194.72',
	...
	def get_service_info(self):
		request = ctypes.c_char_p(b"<request><value>queue_size</value>
			<value>queue_mem_used</value><value>version</value></request>")
		...
		if i is 0: return response.value.decode('utf-8') #0 is success code; output the result
			...
			response.value.decode('utf-8')) #non-0 code signifies failure, we return the xml error comment
	
	def initialize(self, path='.\\Logs\\\0', level=3):
	...
	def send_command(self, xml_command):
		...
		self.conn.SendCommand.restype = ctypes.c_char_p #function returns a pointer to a string
...
class Trader():
	...
	#receives a raw server response and processes it
		...
		#orders = soup.find_all('order')
		#if len(orders) is not 0:
			#for order in orders:
				#self.sort_orders(order)
		...
		if portfolio:
			...
			self.pnl = float(str(portfolio.pnl_income.string))
		return ctypes.c_bool(True)
	...
	def decide(self):
		...
		if not self.positions_open:
			...
			if len(self.usd_open) >= 10 and self.positions_open == 'long' and self.angle_analyse(self.usd_open, 4) < 0:
				# <0 is not conservative, could go for more
				self.trade(-1, 'close')
				self.positions_open = False
				self.usd_open.nullify()
				print('close long at %s' % self.usd[-1])
			elif len(self.usd_open) >= 10 and self.positions_open == 'short' and self.angle_analyse(self.usd_open, 4) > 0:
				self.trade(1,'close')
				self.positions_open = False
				self.usd_open.nullify()
	...
	#instance for further tracking
		result = self.conner.order(type, board=self.conner.securities['dep_board'], 
			seccode=self.conner.securities['dep_seccode'], client=self.conner.account['trade_client'])
	...
	def cancel_orders(self):
		for o in self.conner.orders:
			soup = BeautifulSoup(o, 'html5lib')
			self.conner.cancel_order(str(soup.find('transactionid').string))
...
class Interface():
	...
	def make_menu(self):
		...
		self.menu_commands.add_command(label='Connect', 
			...
			password=self.conner.account['password'], host=self.conner.account['HOST'], 
			port=self.conner.account['PORT']))
	...
	def make_elements(self):
		...
		self.gui_btn_cancel = ttk.Button(self.win, text='cancel orders')
	...
	def make_bindings(self):
		...
		self.gui_btn_cancel.bind('<Button-1>', lambda e:self.tinst.cancel_orders())
	...
	def show_info(self):
		...
		self.gui_account.configure(text="Balance: %s RUB\nToday's income: %s RUB" % (self.tinst.balance, self.tinst.pnl))
	...
	def make_grid(self):
		...
		for i in range(6,8):
			self.win.rowconfigure(i,weight=2)
		self.gui_l1.grid(row=0, column=0, rowspan=1, columnspan=1, sticky=tk.W+tk.E+tk.S+tk.N)
		self.gui_l2.grid(row=0, column=1, rowspan=1, columnspan=1, sticky=tk.W+tk.E+tk.S+tk.N)
		self.gui_oil.grid(row=1, column=0,rowspan=4, columnspan=1, sticky=tk.W+tk.E+tk.S+tk.N)
		self.gui_usd.grid(row=1, column=1,rowspan=4, columnspan=1, sticky=tk.W+tk.E+tk.S+tk.N)
		self.gui_lbl_threshold.grid(row=5, column=0,rowspan=1, columnspan=1, sticky=tk.W+tk.E+tk.S+tk.N)
		self.gui_ent_threshold.grid(row=5, column=1, rowspan=1, columnspan=1, sticky=tk.W+tk.E+tk.S+tk.N)
		self.gui_rec_pos.grid(row=6, column=0, columnspan=2, rowspan=2, sticky=tk.W+tk.E+tk.S+tk.N)
		self.gui_btn_trade.grid(row=8, column=0, columnspan=2, rowspan=1, sticky=tk.W+tk.E+tk.S+tk.N)
		self.gui_orders.grid(row=0,column=2,rowspan=3,columnspan=1, sticky=tk.W+tk.E+tk.S+tk.N)
		self.gui_positions.grid(row=3,column=2,rowspan=1,columnspan=1, sticky=tk.W+tk.E+tk.S+tk.N)
		self.gui_btn_close.grid(row=5, column=2, columnspan=1, rowspan=1, sticky=tk.W+tk.E+tk.S+tk.N)
		self.gui_record.grid(row=6, column=2,rowspan=1,columnspan=1, sticky=tk.W+tk.E+tk.S+tk.N)
		self.gui_account.grid(row=7, column=2,rowspan=1,columnspan=1,sticky=tk.W+tk.E+tk.S+tk.N)
		self.gui_btn_stop.grid(row=8, column=2, columnspan=1, rowspan=1, sticky=tk.W+tk.E+tk.S+tk.N)
		self.gui_btn_cancel.grid(row=4,column=2,sticky=tk.W+tk.E+tk.S+tk.N)
